...
var map = new mapboxgl.Map({
  container: "map",
  style: "mapbox://styles/df13/ck2iz10qk2sn91ds079eu7b0p",
  center: [0.180318, 0],
  zoom: 1,
  minZoom: 1,
  hash: true,
...
var dataSources = [
  {
    ...
    marker_type: "marker_cities",
    type: "cities",
    zoom: 10,
  },
  {
    url: "data/places.json",
    marker_type: "marker_places",
    type: "Places",
    zoom: 13.5,
  },
  {
...
function forwardGeocoder(query) {
  ...
  filterData = filterData.filter((data) => {
    if (
      data.properties.title.toLowerCase().search(query.toLowerCase()) !== -1
    ) {
      return data;
    }
  });
  ...
  filterData.forEach((data) => {
    if (
      ...
      list.innerHTML =
        data.properties.title + ", " + "<small>" + data.properties.type;
      +"</small>";
...
function flyToMarker(e) {
  ...
  searchMarker = new mapboxgl.Marker(markerHighlight).setLngLat(coordinates);
...
// remove the marker from map after
